...
const AgreementPdfViewer = dynamic(
  () => import("@/components/agreement-pdf-viewer").then((module) => module.AgreementPdfViewer),
  {
    ssr: false,
    loading: () => (
      <div className="flex min-h-[640px] items-center justify-center gap-3 bg-slate-100 p-6 text-sm font-medium text-navy-800" role="status">
        <LoaderCircle aria-hidden className="animate-spin" size={20} />
        Preparing the agreement viewer…
      </div>
    )
  }
);
...
export function AgreementClient({ token }: { token: string }) {
  ...
  const submit = async () => {
    if (!documentReady) {
      setError("Wait until every agreement page is ready before signing.");
      return;
    }
        ...
        <Card className="overflow-hidden p-0">
          ...
          {details?.agreement ? (
            <AgreementPdfViewer
              ...
              pdfSha256={details.agreement.pdfSha256}
              pdfUrl={details.agreement.pdfUrl}
              title={details.agreement.title}
              version={details.agreement.version}
            />
          ) : <div className="p-6">{error ? <ErrorAlert>{error}</ErrorAlert> : null}</div>}
        </Card>
