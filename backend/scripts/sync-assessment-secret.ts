...
async function main() {
  ...
  if (!exists) {
    await client.send(new CreateSecretCommand({ Name: secretId, Description: "Savians Assessments staging runtime credentials", SecretString: secretString, Tags: [{ Key: "Project", Value: "savians-assessments" }, { Key: "Environment", Value: "staging" }] }));
