...
function getConfiguredSchema(): string {
  const databaseUrl = process.env.DATABASE_URL;
  if (!databaseUrl) throw new Error("DATABASE_URL is required");
  const schema = new URL(databaseUrl).searchParams.get("schema") ?? "public";
  if (!/^[a-z_][a-z0-9_]*$/i.test(schema)) throw new Error(`Invalid PostgreSQL schema: ${schema}`);
  return schema;
}
...
async function main() {
  ...
  try {
    ...
    const tables = (await prisma.$queryRaw<Array<{ name: string }>>`SELECT tablename AS name FROM pg_tables WHERE schemaname=${schema} AND tablename LIKE 'assessment_%' ORDER BY tablename`).map((row) => row.name);
